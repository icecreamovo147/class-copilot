...
import { noticeService, dutyService, behaviorService } from '@/services';
...
function NoticeTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['notices', currentCohort?.id, page],
    queryFn: () => noticeService.list(currentCohort!.id, { page, page_size: 20 }),
  ...
  return (
    <div>
      {!isReadonly && (
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditing(null); form.resetFields(); setModalVisible(true); }} style={{ marginBottom: 16 }}>
          新增通知
        </Button>
      )}
...
function DutyTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['duties', currentCohort?.id, page],
    queryFn: () => dutyService.list(currentCohort!.id, { page, page_size: 20 }),
    enabled: !!currentCohort,
  });
...
function BehaviorTab() {
  ...
  const { currentCohort, isReadonly } = useAppStore();
  const [modalVisible, setModalVisible] = useState(false);
  // Behavior tab currently has no edit path; editing state preserved for future use
  const [page, setPage] = useState(1);
  const [form] = Form.useForm();

  const { data, isLoading } = useQuery({
    queryKey: ['behaviors', currentCohort?.id, page],
    queryFn: () => behaviorService.list(currentCohort!.id, { page, page_size: 20 }),
    enabled: !!currentCohort,
  });
  ...
  const columns = [
    ...
    { title: '标题', dataIndex: 'title', key: 'title' },
    { title: '分值', dataIndex: 'score', key: 'score', render: (v: number) => v !== 0 ? <span style={{ color: v > 0 ? '#52c41a' : '#ff4d4f' }}>{v > 0 ? `+${v}` : v}</span> : '-' },
    {
      title: '操作',
      key: 'action',
  ...
  return (
    <div>
      {!isReadonly && (
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { form.resetFields(); setModalVisible(true); }} style={{ marginBottom: 16 }}>
          新增奖惩记录
        </Button>
      )}
      <Table dataSource={data?.data || []} columns={columns} rowKey="id" loading={isLoading} pagination={{ current: page, total: data?.total || 0, onChange: setPage }} size="small" />
